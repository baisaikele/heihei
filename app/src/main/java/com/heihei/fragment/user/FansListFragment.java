 ...
 * @author chengbo
 */
public class FansListFragment extends BaseListFragment implements OnItemClickListener {
	...
	public void onRefreshBegin(PtrFrameLayout frame, boolean byUser) {
		...
		offset = 0;
		if (byUser) {
			mUserPresent.getFansList(uid, UserMgr.getInstance().getUid(), offset, PAGE_SIZE, listResponse, false);
		} else {
			mUserPresent.getFansList(uid, UserMgr.getInstance().getUid(), offset, PAGE_SIZE, listResponse, true);
		}
	}
	...
	public void onLoadMore(LoadMoreContainer loadMoreContainer) {
		...
		offset = data.size();
		mUserPresent.getFansList(uid, UserMgr.getInstance().getUid(), offset, PAGE_SIZE, moreListResponse, false);
	}
			...
			if (errCode == ErrorCode.ERROR_OK) {
				...
				if (data.size() <= 0) {
					if (uid.equals(UserMgr.getInstance().getUid())) {
						showEmptyView(getString(R.string.user_fans_declear));
					} else {
						showEmptyView(getString(R.string.user_fans_declear_other));
					}
				} else {
					dismissEmptyView();
				}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		try {
			...
			if (info != null)
				info.setFansInfo(true, getActivity());
			else {
