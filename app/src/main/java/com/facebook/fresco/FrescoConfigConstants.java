...
public class FrescoConfigConstants {
    ...
    public static abstract class FrescoPreHandleListener extends BaseControllerListener<Object> {
        ...
        public void onSubmit(String id, Object callerContext) {
            if (FrescoImageHelper.log)
            	LogWriter.i("Fresco", "onSubmit");
        }

        public void onFinalImageSet(String id, Object imageInfo, Animatable animatable) {
            if (FrescoImageHelper.log)
            	LogWriter.i("Fresco", "onFinalImageSet");
            if (imageInfo != null && imageInfo instanceof ImageInfo) {
                try {
                    ...
                    if (FrescoImageHelper.log)
                    	LogWriter.i("Fresco", "FrescoPreHandleListener onFinalImageSet  w=" + imageinfo.getWidth()
                                + " h=" + imageinfo.getHeight() + " isGif=" + isGif + " ratio=" + ratio + "\nview="
                                + draweeview);

                } catch (Exception e) {
                	LogWriter.e("Fresco", e.getMessage());
                    e.printStackTrace();
                }
            }
        ...
        public void onFailure(String id, Throwable throwable) {
            if (FrescoImageHelper.log)
            	LogWriter.i("Fresco", "onFailure");
        }

        public void onRelease(String id) {
            if (FrescoImageHelper.log)
            	LogWriter.i("Fresco", "onRelease");
        }

        public void onIntermediateImageSet(String id, Object imageInfo) {
            if (FrescoImageHelper.log)
            	LogWriter.i("Fresco", "onIntermediateImageSet");
        }

        public void onIntermediateImageFailed(String id, Throwable throwable) {
            if (FrescoImageHelper.log)
            	LogWriter.i("Fresco", "onIntermediateImageFailed");
        }
    ...
    public static void autoResizeBitmap(Bitmap srcBitmap, Point needSize) {
        ...
        if (w > 0 && h > 0 && src_w > w && src_h > h && !srcBitmap.isRecycled()) {
            try {
                ...
                bb = null;
                if (FrescoImageHelper.log)
                	LogWriter.i("Fresco", " autoResizeBitmap srcBitmap[" + src_w + "," + src_h + "] change to newBitmap["
                            + w + "," + h + "]");
            } catch (Throwable e) {
