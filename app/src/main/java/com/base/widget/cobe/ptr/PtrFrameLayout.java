...
import com.base.widget.cobe.ptr.util.PtrCLog;
import com.wmlives.heihei.R;

/**
 * This layout view for "Pull to Refresh(Ptr)" support all of the view, you can contain everything you want. support: pull to refresh / release to refresh / auto refresh / keep header view while refreshing / hide header view while refreshing It defines {@link in.srain.cube.views.ptr.PtrUIHandler}, which allows you customize the UI easily.
 */
public class PtrFrameLayout extends ViewGroup {

    public static final int HEADER_CHANGE_POS = 0;//下拉更改坐标的方式
    public static final int HEADER_CHANGE_HEIGHT = 1;//下拉更改高度的方式
    private int headerType = HEADER_CHANGE_HEIGHT;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mHeaderView != null) {
            ...
            if (mPtrIndicator.getHeaderHeight() <= 0)
            {
    ...
    private void measureContentView(View child, int parentWidthMeasureSpec, int parentHeightMeasureSpec) {
        ...
        final int childWidthMeasureSpec = getChildMeasureSpec(parentWidthMeasureSpec, getPaddingLeft()
                + getPaddingRight() + lp.leftMargin + lp.rightMargin, lp.width);
        final int childHeightMeasureSpec = getChildMeasureSpec(parentHeightMeasureSpec, getPaddingTop()
                + getPaddingBottom() + lp.topMargin, lp.height);
    ...
    private void layoutHeader()
    {
        ...
        if (mHeaderView != null) {
            MarginLayoutParams lp = (MarginLayoutParams) mHeaderView.getLayoutParams();
            final int left = paddingLeft + lp.leftMargin;
            int top = paddingTop + lp.topMargin + offsetX - mHeaderHeight;
            final int right = left + mHeaderView.getMeasuredWidth();
            int bottom = top + mHeaderView.getMeasuredHeight();
            
            if (headerType == HEADER_CHANGE_HEIGHT)
            {
...
//        mHeaderView.offsetTopAndBottom(change);
    ...
    private void performRefresh(boolean byUser) {
        ...
        if (mPtrHandler != null) {
            mPtrHandler.onRefreshBegin(this,byUser);
        }
    }
