...
//        Emoji.fromChars("\u0023\u20e3"),
    ...
    public static int getOtherEmojiResource(Context context, String text) {
    	int icon = 0;
    	if( text.isEmpty() )
    		return icon;
        ...
        int unicode = Character.codePointAt(text, 0);
        if ( icon == 0 && unicode > 0xff) {
            icon = getEmojiResource(context, unicode);
        }

        int length = text.length();
        int skip = Character.charCount(unicode);
        if( icon == 0 && skip < length ) {
	    	int followUnicode = Character.codePointAt(text, skip);
     ...
     * @param emojiSize
     */
    public static void addEmojis(Context context, Spannable text, int emojiSize, float linespacing) {
        int length = text.length();
        ...
        for (int i = 0; i < length; i += skip) {
            ...
            if (icon > 0) {
                text.setSpan(new EmojiconSpan(context, icon, emojiSize, linespacing), i, i + skip, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
}
