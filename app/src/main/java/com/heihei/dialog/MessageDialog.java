...
public class MessageDialog extends Dialog implements Observer {
	...
	public MessageDialog(Context context, ImageView text) {
	...
	private OnMessageClickListener mListener = new OnMessageClickListener() {
		...
		public void onAvatar(final int position) {
			...
			if (!clickOlder())
				return;
			UserDialog ud = new UserDialog(ActivityManager.getInstance().peek().getActivity(), msg.user, "", false, UserDialog.USERDIALOG_OPENTYPE_DEFAULE);
			ud.show();
		}
	};
